...
if [ -f "$BAND1_FILE" ]; then
    ...
    #     echo "2. Band13（赤外、R20解像度）で完全なColor2画像を生成中..."
    #     echo "   セグメント: 5500x550ピクセル × 10 = 完全画像: 5500x5500ピクセル"
    #     python3 main.py hsdfile file "$BAND13_FILE" color 2 outpic himawari_full_band13_color2.png outdir "$OUTPUT_DIR"
    #     echo ""

    #     echo "3. Band13（赤外）で完全なBD画像を生成中..."
    #     python3 main.py hsdfile file "$BAND13_FILE" color 1 outpic himawari_full_band13_bd.png outdir "$OUTPUT_DIR"
    #     echo ""
    # fi

    # RGB合成（バンド3,2,1を使用）
    echo "4. RGB合成で完全な自然色画像を生成中（バンド3,2,1）..."
    echo "   Band3 (R05): 22000x2200ピクセル × 10 = 22000x22000ピクセル"
    echo "   Band2 (R10): 11000x1100ピクセル × 10 = 11000x11000ピクセル"
    echo "   Band1 (R10): 11000x1100ピクセル × 10 = 11000x11000ピクセル"
    ...
    if [ -f "$BAND1" ] && [ -f "$BAND2" ] && [ -f "$BAND3" ]; then
        python3 main.py rgbfile red "$BAND3" green "$BAND2" blue "$BAND1" outpic himawari_full_rgb.png outdir "$OUTPUT_DIR"
        echo ""
    else
    ...
    echo "  - Band13 (R20): 5500x5500ピクセル"
    echo "  - RGB合成: 11000x11000ピクセル"
    echo ""
else
